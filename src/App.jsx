...
function Card2({ children, style }) {
  return (
    <div style={{ background: C.c2, borderRadius: 12, padding: 11, marginBottom: 8, ...style }}>
      {children}
    </div>
  )
}
...
function RatingRow({ value, onChange }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', background: C.c2, borderRadius: 10, padding: 8 }}>
      {[0, 1, 2, 3, 4, 5].map(n => (
        <div key={n} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <Radio checked={value === n} onChange={() => onChange(n)} />
          <span style={{ fontSize: 9, color: C.mu }}>{n}{n === 5 ? '★' : ''}</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '24px 0 16px' }}>
      ...
      <div style={{ width: '100%' }}>
        ...
        <div style={{ position: 'relative', marginBottom: 12 }}>
          ...
          <input
            ...
            style={{
              background: C.c2, border: `0.5px solid ${C.bdr}`, borderRadius: 11,
              padding: '11px 13px 11px 38px', color: C.txt, fontSize: 13, width: '100%', fontFamily: FF,
            }}
          />
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '20px 0 14px', width: '100%' }}>
        ...
        <div style={{ fontSize: 10, color: C.mu, letterSpacing: '0.8px', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
          Demo Mode
        </div>
        <div style={{ flex: 1, height: '0.5px', background: 'rgba(255,255,255,0.12)' }} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, width: '100%' }}>
        {/* Client card */}
        <div onClick={onClient} style={{
          display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8,
          background: C.c1, border: `0.5px solid ${C.bdr}`, borderRadius: 14,
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginBottom: 12 }}>
        {[
          ...
          { id: 'prog', icon: 'ti-chart-line',     label: 'My Progress'  },
          { id: 'comm', icon: 'ti-users',           label: 'Community'   },
        ].map(({ id, icon, label }) => (
          <div key={id} onClick={() => nav(id)} style={{
            background: C.c2, borderRadius: 14, padding: '15px 8px',
...
function CheckIn() {
  return (
    <>
      ...
      {/* ── Replace [NUMBER] in href with the actual WhatsApp number ── */}
      <iframe
...
function Progress({ metricsOpen, setMetricsOpen }) {
  return (
    <>
      <div style={{ marginTop: 6, marginBottom: 14 }}>
        <div style={{ fontSize: 21, fontWeight: 600, color: C.txt }}>My Progress</div>
        <div style={{ fontSize: 12, color: C.mu, marginTop: 2 }}>Longevity Programme · Day 34</div>
      </div>
      ...
      <Card>
        ...
        <svg width="100%" height="88" viewBox="0 0 290 88" preserveAspectRatio="none">
          <polyline
            points="10,74 55,66 100,60 145,52 190,44 240,38 290,32"
            fill="none" stroke="#6b5ec4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
          />
          <polyline
            points="10,74 55,66 100,60 145,52 190,44 240,38 290,32 290,88 10,88"
            fill="#4f4399" fillOpacity="0.15"
          />
  ...
  return (
    <>
      ...
      <Card style={{ textAlign: 'center', padding: '32px 20px' }}>
        ...
        <a
          href="REPLACE_WITH_GHL_COMMUNITY_URL"
          target="_blank"
          rel="noreferrer"
          style={{
...
function Contact({ nav }) {
  return (
    <>
      ...
      {/* ── Replace PHONENUMBER with the gym's WhatsApp number (digits only, e.g. 447911123456) ── */}
...
function Profile({ nav, onSignOut }) {
  const programmeRows = [
    { label: 'Start date',       val: '18 April 2026' },
    { label: 'Programme day',    val: 'Day 34'        },
  ]
  const sessionRows = [
    { label: 'Allowed per week', val: '1 session' },
    { label: 'Used this week',   val: '1 / 1'     },
    { label: 'Total attended',   val: '18 sessions' },
  ]
  return (
    <>
      ...
      <Card>
        <FieldLabel>Sessions</FieldLabel>
        {sessionRows.map(({ label, val }, i) => (
          <div key={i}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '9px 0' }}>
              <span style={{ fontSize: 12, color: C.mu }}>{label}</span>
              <span style={{ fontSize: 12, color: C.txt }}>{val}</span>
            </div>
            {i < sessionRows.length - 1 && <Divider />}
          </div>
        ))}
      </Card>
          ...
          <Card key={name} style={{ marginBottom: 9 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {[
                ...
                { key: 'cancel',  label: 'Cancel',  bg: 'rgba(251,191,36,0.12)',  color: C.am, border: 'rgba(251,191,36,0.3)'   },
              ].map(({ key, label, bg, color, border }) => (
                <div
                  key={key}
                  onClick={() => mark(name, key === 'cancel' ? 'cancelled' : key)}
                  style={{
                    flex: 1,
                    background: status === key || (key === 'cancel' && status === 'cancelled') ? bg : 'rgba(255,255,255,0.05)',
                    color:  status === key || (key === 'cancel' && status === 'cancelled') ? color : C.mu,
                    border: `0.5px solid ${status === key || (key === 'cancel' && status === 'cancelled') ? border : 'rgba(255,255,255,0.12)'}`,
...
function CoachClients() {
  ...
  const riskAvCol = { amber: C.am,                     red: C.rd                       }
    ...
    <>
      ...
      <div style={{ position: 'relative', marginBottom: 12 }}>
        ...
        <input
          ...
          style={{
            background: C.c2, border: `0.5px solid ${C.bdr}`, borderRadius: 11,
            padding: '9px 13px 9px 34px', color: C.txt, fontSize: 12, width: '100%', fontFamily: FF,
          }}
        />
      </div>
      ...
      <div style={{ display: 'flex', gap: 5, marginBottom: 14, flexWrap: 'wrap' }}>
        {['All (24)', '🟡 At Risk (3)', '🔴 Flagged (1)'].map((f, i) => (
          <span key={i} style={{
            background: i === 0 ? C.acc : C.c2,
            color:      i === 0 ? C.txt : C.mu,
            borderRadius: 20, padding: '4px 11px', fontSize: 10, cursor: 'pointer',
            border: i > 0 ? `0.5px solid ${C.bdr}` : 'none', fontFamily: FF,
          }}>
            {f}
          </span>
        ))}
      ...
      {clients.map(({ init, name, info, badge, label, risk }) => (
        <div key={name} style={{
          ...
          <div style={{
            ...
            background: risk ? riskAvBg[risk] : C.acc,
            color: risk ? riskAvCol[risk] : C.txt,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
...
const CLIENT_SCREENS = ['home','ci','book','prog','more','res','comm','contact','prof']
...
const CLIENT_NAV = [
  ...
  { id: 'ci',   icon: 'ti-clipboard-check',label: 'Check-In' },
  { id: 'prog', icon: 'ti-chart-line',     label: 'Progress' },
  { id: 'more', icon: 'ti-dots',           label: 'More'     },
]

// Sub-screens (res, comm, contact, prof) keep the More tab active (index 4)
const CLIENT_NAV_IDX = {
  home: 0, book: 1, ci: 2, prog: 3,
  more: 4, res: 4, comm: 4, contact: 4, prof: 4,
}
...
export default function App() {
  ...
  return (
    <div style={{
      minHeight: '100vh', background: C.bg, fontFamily: FF, color: C.txt,
      ...
      <div style={{
        ...
        padding: '12px 22px 6px', fontSize: 12, fontWeight: 600, flexShrink: 0,
      }}>
        <span>9:41</span>
      ...
      <div style={{ padding: '0 16px', paddingBottom: showClient || showCoach ? 90 : 24 }}>
        ...
        {screen === 'prog'    && <Progress    metricsOpen={metricsOpen} setMetricsOpen={setMetricsOpen} />}
      ...
      {showClient && (
        <div style={{
          ...
          borderTop: `0.5px solid ${C.bdr}`, display: 'flex',
          ...
          {CLIENT_NAV.map(({ id, icon, label }, i) => (
            <div
              ...
              style={{
                ...
                gap: 3, cursor: 'pointer', fontSize: 10, fontFamily: FF,
                color: CLIENT_NAV_IDX[screen] === i ? C.txt : C.mu,
              }}
            >
      ...
      {showCoach && (
        <div style={{
          ...
          <div
            ...
            style={{
              flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center',
              gap: 3, cursor: 'pointer', fontSize: 10, fontFamily: FF, color: C.mu,
            }}
          >
